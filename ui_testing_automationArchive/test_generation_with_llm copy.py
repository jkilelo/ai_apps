...
try:
    ...
    from prompts import PromptEngine, PromptStrategy, PromptRequest, StrategyOrchestrator
...
class QuantumTestGenerator:
    ...
    async def _generate_context(
        ...
        # Determine page type
        form_elements = [e for e in elements if e.element_type in ["input", "textarea", "select"]]
        ...
        for elem in interactive_elements[:10]:  # Top 10 interactive elements
            if elem.element_type == "button":
                ...
                context.key_actions.append(f"Enter data in {elem.attributes.get('placeholder', 'field')}")
    ...
    async def _apply_opro_optimization(
        ...
        for iteration in range(self.config.opro_iterations):
            ...
            prompt_request = PromptRequest(
                ...
                Previous understanding: {[e.ai_analysis.semantic_role for e in elements[:5]]}
                ...
                """,
                context={"elements": [e.to_dict() for e in elements[:10]]}
            )
            
            # Get optimized understanding
            optimized_prompt = self.prompt_engine.generate(prompt_request)
    ...
    def _format_elements_for_prompt(self, elements: List[EnhancedElement]) -> str:
        ...
        for i, elem in enumerate(elements, 1):
            formatted.append(
                f"{i}. {elem.element_type}: {elem.text or 'no text'} "
                f"[{elem.selectors or elem.xpath}]"
            )
        return "\n".join(formatted)
    ...
    def _organize_into_features(
        ...
        for category, category_scenarios in features_by_category.items():
            feature = GherkinFeature(
                ...
                url=url,
                ai_metadata={
                    "strategies": list(self.metrics["strategies_applied"]),
    ...
    async def generate_from_url(
        ...
        if extract_elements:
            ...
            try:
                ...
                extractor = ElementsExtractorWithLLM(llm_provider=self.llm_provider)
                result = await extractor.extract_from_url(url)
                elements = result.elements
            except ImportError:
                logger.warning("Element extractor not available, using mock elements")
...
async def example_2_ecommerce_test_generation():
    ...
    config = TestGenerationConfig(
        ...
        num_consistency_samples=7,  # More samples for better consistency
        opro_iterations=5,  # More iterations for better optimization
        test_categories=[
        ...
        for elem in ecommerce_elements:
            enhanced_elem = EnhancedElement(
                ...
                ai_analysis=AIAnalysis(
                    semantic_role=f"{elem['element_type']} element",
                    functional_purpose=f"Used for {elem.get('text', 'interaction')}",
...
async def main():
    ...
    if not api_keys_available:
        ...
        print("\nRunning with mock LLM responses...")
