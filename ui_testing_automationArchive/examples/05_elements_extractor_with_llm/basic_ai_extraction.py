...
try:
    ...
    print("[OK] AI extraction module imported successfully")
...
async def example_1_basic_ai_extraction():
    ...
    try:
        ...
        print(f"  [OK] DOM-only completed in {dom_result.extraction_time:.2f}s")
        print(f"  📊 Elements found: {len(dom_result.elements)}")
        print(f"  🎯 Success: {dom_result.success}")
        
    except Exception as e:
        print(f"  [ERROR] DOM-only extraction failed: {e}")
        dom_result = None
    ...
    try:
        ...
        print(f"  [OK] AI-enhanced completed in {ai_result.extraction_time:.2f}s")
        ...
        print(f"  [ERROR] AI-enhanced extraction failed: {e}")
        ai_result = None
...
async def example_2_semantic_context_extraction():
    ...
    try:
        ...
        print(f"  [OK] Context-aware extraction completed")
        print(f"  [TIME] Time: {result.extraction_time:.2f}s")
        ...
        logger.error(f"[ERROR] Context-aware extraction failed: {e}")
        return None
...
async def example_3_multi_strategy_analysis():
    ...
    for config in strategy_configs:
        ...
        try:
            ...
            print(f"  [TIME] Time: {strategy_result['time']:.2f}s")
            ...
            logger.error(f"  [ERROR] Strategy {config['name']} failed: {e}")
            results.append({
                "name": config["name"],
                "success": False,
                "error": str(e)
...
async def example_4_ai_element_insights():
    ...
    try:
        ...
        print(f"🧠 AI capabilities: Semantic [OK], Visual [OK], Context Learning [OK]")
        
        result = await extractor.extract_from_url(test_url)
        
        print(f"[OK] Analysis completed in {result.extraction_time:.2f}s")
        ...
        logger.error(f"[ERROR] AI insights analysis failed: {e}")
        return None
...
async def example_5_llm_provider_comparison():
    ...
    for provider, name in providers_to_test:
        ...
        try:
            ...
            print(f"  [OK] Success: {provider_result['success']}")
            print(f"  [TIME] Total time: {provider_result['time']:.2f}s")
            ...
            logger.error(f"  [ERROR] Provider {name} failed: {e}")
    ...
    for result in provider_results:
        if result["success"]:
            print(f"{result['provider']:<18} {'[OK] PASS':<10} {result['time']:<10.2f} {result['elements']:<10} {result['ai_elements']:<12} {result['avg_confidence']:<12.2%}")
        else:
            print(f"{result['provider']:<18} {'[ERROR] FAIL':<10} {'-':<10} {'-':<10} {'-':<12} {'-':<12}")
    ...
    if successful_results:
        ...
        for result in successful_results:
            if result["time"] == best_time:
                print(f"  [FAST] Fastest: {result['provider']} ({result['time']:.2f}s)")
...
async def main():
    ...
    print(f"  [FAST] Multi-provider LLM support")
