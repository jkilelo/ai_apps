    ...
    async def extract_from_url(
        ...
        try:
            ...
            browser_result = await self.browser.extract_and_analyze(
                url=url,
                custom_prompt=custom_prompts.get('main') if custom_prompts else None
            )
            ...
            if not browser_result.success:
                return ExtractionResult(
                    ...
                    extraction_stats={},
                    success=False,
                    errors=[browser_result.error] if browser_result.error else ["Extraction failed"]
                )
            
            # Convert browser elements to enhanced elements
            enhanced_elements = await self._enhance_elements(
                browser_result.elements_with_context,
                custom_prompts
            )
