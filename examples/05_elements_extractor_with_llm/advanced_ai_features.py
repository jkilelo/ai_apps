...
try:
    ...
    print("✅ Advanced AI modules imported successfully")
...
async def example_1_prompt_strategy_optimization():
    ...
    for strategy, description in strategies_to_test:
        ...
        try:
            ...
            print(f"  ⏱️ Processing time: {strategy_result['processing_time']:.2f}s")
    ...
    if successful:
        ...
        for result in successful:
            ...
            if result["processing_time"] == best_speed:
                print(f"  ⚡ Fastest processing: {result['strategy']} ({result['processing_time']:.2f}s)")
...
async def example_2_visual_ai_analysis():
    ...
    try:
        ...
        print(f"✅ Visual analysis completed in {result.extraction_time:.2f}s")
...
async def example_3_context_learning_adaptation():
    ...
    try:
        ...
        for i, url in enumerate(test_urls, 1):
            ...
            print(f"  ⏱️ Processing: {learning_result['processing_time']:.2f}s")
        ...
        logger.error(f"❌ Context learning analysis failed: {e}")
        return []
...
async def example_4_accessibility_ai_analysis():
    ...
    print(f"♿ Accessibility Analysis Configuration:")
    ...
    try:
        ...
        print(f"♿ AI-powered accessibility assessment")
        
        result = await extractor.extract_from_url(test_url)
        
        print(f"✅ Accessibility analysis completed in {result.extraction_time:.2f}s")
        ...
        if accessibility_insights["low_accessibility"]:
            ...
            for i, element in enumerate(accessibility_insights["low_accessibility"][:3], 1):
                ...
                print(f"  {i}. {element.element_type.value}: {element.text[:40]}...")
                print(f"     ♿ Accessibility score: {analysis.accessibility_score:.2%}")
                ...
                if present_attrs:
                    print(f"     ✅ Present attributes: {', '.join(present_attrs)}")
                if missing_attrs and element.is_interactive:
                    print(f"     ❌ Missing attributes: {', '.join(missing_attrs[:3])}")
        ...
        if total_elements_with_scores:
            ...
            if overall_score >= 0.8:
                print(f"  ✅ Status: EXCELLENT - Likely WCAG compliant")
        ...
        logger.error(f"❌ Accessibility AI analysis failed: {e}")
        return None
...
async def example_5_performance_optimization():
    ...
    try:
        ...
        print(f"  ⏱️ Cold start time: {cold_time:.2f}s")
        ...
        print(f"  ⏱️ Warm extraction time: {warm_time:.2f}s")
        ...
        print(f"  ⏱️ Speed-optimized time: {speed_time:.2f}s")
        print(f"  📊 Elements: {len(result3.elements)}")
        print(f"  🧠 AI elements: {performance_data[2]['ai_elements']}")
        print(f"  ⚡ Speed focus: Minimal AI processing")
        ...
        print(f"  ⚡ Speed optimization: {speed_vs_cold:.2f}x faster than full AI")
