...
from web_automation_portable_v2.backend.data_types import (
    Element,
    ExtractionResult,
    ExtractionConfig,
    PageAnalysis,
...
from web_automation_portable_v2.backend.browser import UltimateStealthBrowser
from web_automation_portable_v2.backend.elements_extractor_no_llm import (
    ElementsExtractorNoLLM,
)
from web_automation_portable_v2.backend.elements_extractor_with_llm import (
...
try:
    from web_automation_portable_v2.backend.test_generation_with_llm import (
...
class PipelineOrchestrator:
    ...
    async def process_url(
        self, url: str, generate_tests: bool = True, max_elements: int = 10
    ) -> Dict[str, Any]:
        """
        ...
        results = {"url": url, "stages": {}, "success": False, "errors": []}
        ...
        try:
            ...
            logger.info(
                f"[STAGE 1] Complete: {len(raw_elements)} elements in {time.time() - stage1_start:.2f}s"
            )
            ...
            if raw_elements:
                ...
                enriched = [
                    self.no_llm_extractor._enrich_element(e) for e in raw_elements
                ]
            ...
            logger.info(
                f"[STAGE 2] Complete: {len(filtered)} filtered elements in {time.time() - stage2_start:.2f}s"
            )
            ...
            interactive = [e for e in filtered if e.is_clickable or e.is_editable][
                :max_elements
            ]
            ...
            if interactive:
                try:
                    enriched_elements = await self.llm_analyzer.analyze_elements(
                        interactive
                    )
            ...
            logger.info(
                f"[STAGE 3] Complete: {len(enriched_elements)} enriched in {time.time() - stage3_start:.2f}s"
            )
            ...
            if generate_tests and self.test_generator:
                ...
                try:
                    test_suite = await self.test_generator.generate_test_scenarios(
                        page_analysis, categories=None  # Use defaults
                    )

                    results["stages"]["test_generation"] = {
                        "duration": time.time() - stage4_start,
                        "scenario_count": (
                            len(test_suite.scenarios)
                            if hasattr(test_suite, "scenarios")
                            else 0
                        ),
                        "success": True,
                    }
                    ...
                    logger.info(
                        f"[STAGE 4] Complete: Tests generated in {time.time() - stage4_start:.2f}s"
                    )
                except Exception as e:
            ...
            logger.info(
                f"[PIPELINE] Complete in {new_time:.2f}s (vs {old_time:.2f}s old way)"
            )
            logger.info(
                f"[PIPELINE] Performance improvement: {results['performance_improvement']}"
            )
