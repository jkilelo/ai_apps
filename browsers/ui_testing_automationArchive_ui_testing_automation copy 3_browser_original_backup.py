...
# Import Pydantic contracts
from browser_contracts import (
    StealthLevel, ExtractionStrategy,
    StealthConfig,
    ElementData, ExtractionResult,
    ElementType
)
...
class UltimateStealthBrowser:
    ...
    def _get_launch_options(self) -> Dict[str, Any]:
        ...
        # Use browser_config.py for all configuration
        if HAS_BROWSER_CONFIG:
            try:
                # Convert StealthLevel to string for browser_config
                stealth_level = self.config.level.value if hasattr(self.config.level, 'value') else str(self.config.level)
                
                # Get complete configuration from browser_config
                full_config = get_browser_config(stealth_level.lower())
            ...
            # Minimal fallback if browser_config.py is not available
            logger.warning("browser_config.py not available, using minimal configuration")
            return {
    ...
    async def _create_stealth_context(self) -> BrowserContext:
        ...
        # Try to use enhanced configuration from browser_config.py
        if HAS_BROWSER_CONFIG:
            try:
                # Convert StealthLevel to string for browser_config
                stealth_level = self.config.level.value if hasattr(self.config.level, 'value') else str(self.config.level)
                
                # Get context options from browser_config
                full_config = get_browser_config(stealth_level.lower())
                ...
                logger.error(f"Failed to create context with browser_config: {e}")
                raise
        
        # Minimal fallback if browser_config.py is not available
        logger.warning("browser_config.py not available, using minimal context")
