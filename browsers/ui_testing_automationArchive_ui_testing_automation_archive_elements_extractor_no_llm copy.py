...
class ElementsExtractorNoLLM:
    ...
    def _convert_browser_elements(
        ...
        for be in browser_elements:
            ...
            is_clickable = (
                be.tag_name in ["button", "a", "input", "select"] or
                be.attributes.get("onclick") is not None or
                be.attributes.get("role") == "button" or
                element_type in [ElementType.BUTTON, ElementType.LINK]
            )
            
            # Determine if editable
            is_editable = (
                be.tag_name in ["input", "textarea", "select"] or
                be.attributes.get("contenteditable") == "true" or
                element_type in [ElementType.INPUT, ElementType.TEXTAREA, ElementType.SELECT]
            )
    ...
    def _generate_selectors(self, elements: List[ExtractedElement]) -> List[ExtractedElement]:
        ...
        for element in elements:
            ...
            if element.selector:
                ...
                is_xpath = element.selector.startswith('//') or element.selector.startswith('/')
                selectors.append(ElementSelector(
