...
# Import browser automation (Layer 0)
...
# Import data contracts from elements_extractor_no_llm
...
# Import LLM capabilities (Layer 0 - single source of truth)
from base.llm import call_default_llm, LLMResponse

# Import prompt engineering capabilities (Layer 0)
...
# ============================================================================
# ENHANCED DATA MODELS (Integration Layer - Pydantic v2)
# ============================================================================
...
class LLMAnalysisResult(BaseModel):
    ...
    analysis_time: float = Field(default=0.0, ge=0.0)
...
class SemanticContext(BaseModel):
    ...
    page_type: Optional[str] = Field(default=None, description="e-commerce, blog, form, etc.")
    page_purpose: Optional[str] = Field(default=None)
    main_functionality: List[str] = Field(default_factory=list)
    user_journey_stage: Optional[str] = Field(default=None)
    key_interactions: List[str] = Field(default_factory=list)
    business_domain: Optional[str] = Field(default=None)
    technical_stack: List[str] = Field(default_factory=list)
    accessibility_features: List[str] = Field(default_factory=list)

class ElementWithLLMContext(BaseModel):
    """Enhanced element with LLM-derived context"""
    
    model_config = ConfigDict(str_strip_whitespace=True)
    
    element_data: ExtractedElement = Field(..., description="Original extracted element")
    semantic_role: Optional[str] = Field(default=None)
    functional_purpose: Optional[str] = Field(default=None)
    test_priority: float = Field(default=0.0, ge=0.0, le=1.0)
...
class BrowserWithLLMConfig(BaseModel):
    ...
    llm_provider: str = Field(default="default", description="Uses default from llm.py")
    max_tokens: int = Field(default=4000, gt=0)
    temperature: float = Field(default=0.3, ge=0.0, le=2.0)
    
    # Prompt configuration
    enable_prompt_optimization: bool = Field(default=True)
    prompt_strategy: Optional[PromptStrategy] = Field(default=None)
    task_type: TaskType = Field(default=TaskType.ANALYTICAL)
    complexity_level: ComplexityLevel = Field(default=ComplexityLevel.MODERATE)
    
    # Analysis configuration
    analyze_semantics: bool = Field(default=True)
    analyze_accessibility: bool = Field(default=True)
    analyze_security: bool = Field(default=True)
    generate_test_suggestions: bool = Field(default=True)
    
    # Performance configuration
    cache_llm_results: bool = Field(default=True)
...
class ExtractionWithLLMResult(BaseModel):
    """Complete extraction result with LLM analysis"""
    
    model_config = ConfigDict(str_strip_whitespace=True)
    
    url: str = Field(..., description="URL that was analyzed")
    browser_result: ExtractionResult = Field(..., description="Original browser extraction result")
    elements_with_context: List[ElementWithLLMContext] = Field(default_factory=list)
    semantic_context: Optional[SemanticContext] = Field(default=None)
    llm_analysis: Optional[LLMAnalysisResult] = Field(default=None)
...
class BrowserWithLLM:
    ...
    async def extract_and_analyze(
        ...
        placeholder_browser_result = ExtractionResult(
            url=url,
            success=False,
            elements=[],
            page_title="",
    ...
    async def _generate_page_insights(
        self,
        browser_result: ExtractionResult,
    ...
    def _create_element_analysis_prompt(self, elements: List[ExtractedElement]) -> str:
    ...
    def _create_insights_prompt(
        ...
        context_info = semantic_context.model_dump() if semantic_context else {}
