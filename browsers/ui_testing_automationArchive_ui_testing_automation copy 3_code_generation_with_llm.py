from pipeline_contracts import GherkinStep
...
from prompts import (
    PromptEngine, PromptStrategy, PromptRequest, 
    StrategyOrchestrator, TaskType, ComplexityLevel
)

# Import test generation components if available
try:
    from test_generation_with_llm import (
        TestScenario, GherkinStep, GherkinFeature, TestCategory
    )
    logger.info("[OK] Successfully imported test_generation_with_llm")
except ImportError:
    # Define locally if not available (for standalone operation)
    @dataclass
    @dataclass 
    class TestScenario:
        name: str
...
# No need for LLMProvider - using default LLM from llm.py

logger.info("[OK] All imports configured for real LLM usage")
...
class QuantumCodeGenerator:
    ...
    async def _generate_usc_paths(
        ...
        for focus_name, focus_prompt in focuses:
            ...
            prompt = prompt_response.enhanced_prompt
