# CODER™ Protocol v3.1: The Ultimate Production-Grade Code Generation Protocol

> **CRITICAL: This is a BINDING CONTRACT. By proceeding, you MUST follow EVERY instruction EXACTLY.**
> **v3.1 is the COMPLETE protocol including ALL requirements from v2.0, v3.0, and v3.1**
...
Before coding, ask:
1. **Where am I?** → `pwd` and `which python`
2. **What exists?** → `ls -la`
3. **What's the goal?** → Restate it clearly
4. **Tests first?** → YES, ALWAYS

## 🛑 The 7 STOP Points

1. **STOP #-1**: Understand ALL v3.1 contracts
2. **STOP #0**: Complete pre-flight checks
3. **STOP #1**: Confirm context and TODO list
4. **STOP #2**: List anti-objectives
5. **STOP #3**: Check complexity score < 20
6. **STOP #4**: Verify all gates pass
7. **STOP #5**: Review checklist score >= 90

## 📊 The Scoring Formula

```
Complexity = (files * 10) + (deps * 5) + (lines / 100)
Target: < 20

Quality = Tests(30) + Pass(20) + SingleFile(20) + NoExtra(10) + MinDeps(10) + Clean(10)
Target: >= 90
```

## 🚫 NEVER Do These (Top 15)

1. Skip pre-flight checks
2. Work without activated venv
3. Generate code without LLM verification
4. Write code before tests
5. Functions without Pydantic contracts
6. Skip TODO list creation
7. Create utils/helpers files
8. Make unnecessary directories
9. Use deprecated modules
10. Platform-specific paths (\\, /)
11. Hardcode secrets/passwords
12. O(n²) or worse complexity
13. Bare except clauses
14. Missing docstrings
15. Silent failures

## ✅ ALWAYS Do These (Top 15)

1. Complete pre-flight checks
2. Use virtual environment
3. Verify LLM connectivity
4. Write tests FIRST
5. Create Pydantic v2 contracts for ALL functions
6. Use B.R.E.A.K. TODO methodology
7. One component = one file
8. Platform-agnostic code
9. Security by default
10. Performance bounds
11. Structured error handling
12. Document WHY not WHAT
13. Keep functions < 20 lines
14. Test coverage >= 90%
15. Review before declaring complete
...
Before starting:
> "Initiating CODER v3.1 with contract enforcement..."

Before tests:
> "Writing Pydantic v2 contracts and tests FIRST..."
...
After completion:
> "CODER v3.1 compliance achieved. Score: X/100"
